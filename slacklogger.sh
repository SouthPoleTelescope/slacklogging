 ...
 then
	printf "Performing software updates/installs to make sure you have everything we need, then we'll get started.\n"
		...
		cd ${DIR}/slack-history-export
	  	npm install . 1>${DIR}/setup.log 2>&1
	printf "\n"
	exit 200
fi

# Housecleaning
rm -r ${static_build}/* &>>${DIR}/setup.log
cd $json_exports;
rm -r ${json_exports}/* &>>${DIR}/setup.log
...
curl -s --output users.json --url https://slack.com/api/users.list?token=${slack_token}&pretty=1 &>${DIR}/curlusers.log;
curl -s --output channels.json --url https://slack.com/api/channels.list?token=${slack_token}&pretty=1 &>${DIR}/curlchannels.log;
sleep 10s;
...
wget -q -P ${static_build} -mpck --user-agent="" -e robots=off --wait 1 -E -p -k http://localhost:5000 &>${DIR}/static_wget.log;
