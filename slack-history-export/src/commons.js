...
function saveDataAsJSON(data, args, filename) {
  return new Promise((resolve, reject) => {
    ...
    const filePath = (args.filename) ? (/\.json$/.test(args.filename)) ? `${currentDir}/${args.filename}` : `${currentDir}/${args.filename}.json` : `${currentDir}/${filename}.json`;
...
function saveDataAsCSV(data, args, filename) {
  return new Promise((resolve, reject) => {
    const currentDir = args.directory || process.cwd();
    const filePath = (args.filename) ? (/\.csv$/.test(args.filename)) ? `${currentDir}/${args.filename}` : `${currentDir}/${args.filename}.csv` : `${currentDir}/${filename}.csv`;
